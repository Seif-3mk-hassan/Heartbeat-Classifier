"""
Weighted KNN ECG Classifier
Uses class weights and distance weighting to handle imbalanced data
"""
...
def plot_confusion_matrix(conf_matrix, labels, save_path='confusion_matrix_weighted.png'):
    """Plot and save confusion matrix"""
    plt.figure(figsize=(10, 8))
    sns.heatmap(conf_matrix, annot=True, fmt='d', cmap='Greens', 
                xticklabels=labels, yticklabels=labels)
    plt.title('Confusion Matrix - Weighted KNN', fontsize=16, fontweight='bold')
...
def train_weighted_knn(train_features, train_labels, k=5, weights='distance'):
    """
    ...
    Args:
        ...
        weights: 'uniform' or 'distance' for neighbor weighting
    ...
    # Train weighted KNN classifier
    print(f"   Training Weighted KNN (k={k}, weights='{weights}')...")
    classifier = KNeighborsClassifier(n_neighbors=k, weights=weights, metric='euclidean')
...
def main():
    """Run weighted KNN classification"""
    print("="*60)
    print("WEIGHTED KNN ECG CLASSIFICATION")
    print("="*60)
    ...
    for dataset_name, beat_types in datasets:
        ...
        # Train weighted KNN
        print("\n4. Training with distance weighting...")
        classifier = train_weighted_knn(train_features, dataset['train']['labels'], 
        ...
        save_path = os.path.join(os.path.dirname(__file__), 
                                 f'confusion_matrix_weighted_{dataset_name.replace("&", "_")}.png')
        plot_confusion_matrix(conf_matrix, labels, save_path)
    
    # Save results to file
    print("\n" + "="*60)
    print("Saving results...")
    results_file = os.path.join(os.path.dirname(__file__), 'weighted_results.txt')
    with open(results_file, 'w') as f:
        f.write("Weighted KNN ECG Classification Results\n")
        f.write("="*60 + "\n\n")
    ...
    print("\n" + "="*60)
    print("Weighted KNN Classification completed successfully!")
    print("="*60)
